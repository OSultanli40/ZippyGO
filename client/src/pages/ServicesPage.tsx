...
export default function ServicesPage() {
  ...
  const handleBook = (serviceName: string) => {
    toast({
      ...
      description: `We've sent a request for ${serviceName}. You will receive a confirmation soon.`,
      duration: 3000,
    });
  };
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredServices.map((service) => (
          <Card key={service.id} className="overflow-hidden border-border shadow-sm hover:shadow-md transition-shadow">
            ...
            <CardHeader className="pb-2">
              <div className="flex justify-between items-start">
                ...
                {service.available ? (
                  ...
                  <span className="text-xs font-bold text-red-600 bg-red-50 px-2 py-1 rounded-full">Booked</span>
                )}
              </div>
              <CardTitle className="text-xl font-bold">{service.name}</CardTitle>
            </CardHeader>
            <CardContent>
              <p className="text-muted-foreground text-sm mb-4 min-h-[60px]">{service.description}</p>
              <div className="flex items-center gap-2 text-sm font-medium">
                <ShieldCheck className="h-4 w-4 text-primary" /> Verified Professional
              </div>
            </CardContent>
            <CardFooter className="border-t bg-secondary/10 pt-4 flex items-center justify-between">
              ...
              <Button onClick={() => handleBook(service.name)} disabled={!service.available}>
                {t("services.book")}
              </Button>
            </CardFooter>
          </Card>
